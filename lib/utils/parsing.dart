...
class Parsing {
  ...
  static dynamic? numFrom(dynamic data, {double defaultValue = 0}) {

    if (null == data) return defaultValue;
    if (data is double) return (data * 100).truncateToDouble() / 100;
    if (data is int) return data;
  ...
  static bool boolFrom(dynamic data, {bool defaultValue = false}) {
    ...
    if (data is String) {
      return (data == "1" || data.toLowerCase() == "true" || data.toLowerCase() == "yes") ? true : defaultValue;
    }
    return defaultValue;
  }
  /// Get Double from Dynamic Data
